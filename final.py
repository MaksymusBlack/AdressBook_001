...
##  Відкриття необхідного файлу
    def open(self, file_path):
        with open(file_path, 'rb') as file:
            self.data = pickle.load(file)
##  Закриття файла зі збереженням
    ...
    def find(self, data):
        ...
        if data.isdigit():
            for key, value in self.data.items():
                for num in value.phones:
                    if data in num.to_str():
                        found_records.append((key, value))

    ## Перевірка по імені
        if data.isalpha():
            for key in self.data.keys():
                if data in key:
                    found_records.append((key, self.data[key]))
